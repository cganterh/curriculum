\documentclass[11pt,letterpaper,sans]{moderncv}
  \usepackage[scale=0.85]{geometry}

  \moderncvstyle{fancy}
  \moderncvcolor{purple}
  \nopagenumbers{}
  \setlength{\hintscolumnwidth}{3.51cm}

  \name{Cristóbal}{Ganter}
  \phone[mobile]{+56~9~4387~9292}
  \email{cganterh@gmail.com}
  % \homepage{www.cganterh.net}
  \social[linkedin]{cganterh}
  \social[github]{cganterh}

\begin{document}
  \makecvtitle{}

  \section{Idiomas}
    \cvitem{Español}{Idioma materno}
    \cvitem{Inglés}{Avanzado}
    \cvitemwithcomment{Alemán}{Medio}
      {Sprachdiplom nivel 2 aprobado.}
    \cvitem{Italiano}{Bajo}

  \section{Educación}
    \cventry{2006--2016}{Ingeniero Civil Electrónico}
      {Universidad Técnica Federico Santa María}{Valparaíso}
      {}
      {Mención Computadores, Sub-Mención Informática}

    % \cventry{1994--2005}{Educación Básica y Media}
    %   {Colegio Alemán de Santiago}{Santiago}{}
    %   {}

  \section{Experiencia}
    \cventry{2016--presente}{R\&D Engineering, Staff Engineer}
    {Synopsys}{Santiago}{}
    {
      Actividades realizadas:
      \begin{itemize}
        \item Diseño e implementación de software en Python y C++.
        \item Diseño e implementación de aplicaciones full-stack (MongoDB, Python, Javascript).
        \item Implementación de algoritmos de grafos.
        \item Desarrollo de imagenes de Docker para desarrollo de software y distribución.
        \item Mantenimiento de software de visualización cientifica.
        \item Diseño y mantención de sistemas de integración continua y despliegue continuo.
        \item Empaquetado y distribución de software.
        \item Supervición y capacitación de colegas nuevos.
      \end{itemize}
    }

    \pagebreak[0]

    \cventry{2016--2016}{Asesor}
    {UTFSM}{Valparaíso}{}
    {
      Asesoría para el proyecto: ``Desarrollo de plataforma
      Interactiva TIC para salas SCALE-UP''.
    }

    \pagebreak[0]

    \cventry{2014--2015}{Freelancer}
      {Reiser Technologies}{Valparaíso}{}
      {
        Desarrollo front-end.
      }

    \pagebreak[0]

    \cventry{2014}{Asesor de MongoDB}
      {Xompass}{Valparaíso}{}
      {
        Asesoría para optimizar la base de datos MongoDB de
        Xompass.
      }

    \pagebreak[0]

    \cventry{2013}{Especialista en Computación}
      {Proyecto FIC SURHGE}{Valparaíso}{}
      {
        Actividades realizadas:
        \begin{itemize}
          \item Administración sistemas Linux.
          \item Análisis de datos.
          \item Programación de aplicación web.
          \item Programación modelos matemáticos.
          \item Machine learning.
        \end{itemize}
      }

    \pagebreak[0]

    \cventry{2011--2012}{Ayudante de Cátedra}
      {UTFSM}{Valparaíso}{}
      {
        \emph{Estructura de Datos y Algoritmos}.\newline{}
        Actividades realizadas:
        \begin{itemize}
          \item Ayudantías presenciales.
          % \item Corrección de pruebas y tareas.            %delete
          \item Desarrollo de taller de programación en C.
          % \item Elaboración de preguntas para pruebas.     %delete
        \end{itemize}
      }

    \pagebreak[0]

    \cventry{2010--2011}{Ayudante de Desarrollo de Software}
      {UTFSM}{Valparaíso}{}
      {
        Actividades realizadas:
        \begin{itemize}
          \item Administración de sistemas Linux.
          \item Desarrollo de software relacionado con probabilidades en Delphi.
          \item Puesta en marcha y mantenimiento de un sistema de control de versiones.
        \end{itemize}
      }

    \pagebreak[0]

    \cventry{2009}{Practicante}
      {Wetland}{Santiago}{}
      {
        Programación de PLCs para su uso en plantas de tratamiento de aguas.
      }

    \pagebreak[0]

    \cventry{2005}{Empleado}
      {CONAC}{Talca}{}
      {
        Diseño de un controlador electrónico de nivel de estanque.
      }
    %
    % \pagebreak[0]
    %
    % \cventry{2002}{Encargado de Aseo}
    %   {Camping Radal}{Reserva Nacional Radal Siete Tazas}{}
    %   {}

\pagebreak[0]

  \section{Conocimientos de Computación}
    \subsection{Sistemas Operativos}
      \begin{cvcolumns}
        \cvcolumn{Muy Avanzado}{Linux}
        \cvcolumn{Avanzado}{Windows}
      \end{cvcolumns}

    \pagebreak[0]

    \subsection{Lenguajes de Programación}
      \begin{cvcolumns}
        \cvcolumn{Avanzado}{
          Python\newline{}
          C++
        }
        \cvcolumn{Medio}{
          C\newline{}
          Bash\newline{}
          JavaScript
        }
        \cvcolumn{Básico}{
          AWK\newline{}
          TCL
        }
      \end{cvcolumns}

    \pagebreak[0]

    \subsection{Otros Lenguajes}
      \begin{cvcolumns}
        \cvcolumn{Avanzado}{
          HTML5\newline{}
          Markdown\newline{}
          JSON\newline{}
          CSS\newline{}
          YAML\newline{}
          Makefile
        }
        \cvcolumn{Medio}{
          XML
        }
        \cvcolumn{Básico}{
          SVG\newline{}
          SQL
        }
      \end{cvcolumns}

    \pagebreak[0]

    \subsection{Frameworks}
      \begin{cvcolumns}
        \cvcolumn{Avanzado}{
          Tornado Web
        }
        \cvcolumn{Básico}{
          Flask\newline{}
        }
      \end{cvcolumns}

    \pagebreak[0]

    \subsection{Herramientas}
      \begin{cvcolumns}
        \cvcolumn{Muy Avanzado}{
          GIT\newline{}
          Inkscape
        }
        \cvcolumn{Avanzado}{
          Docker\newline{}
          Microsoft Office
        }
        \cvcolumn{Medio}{
          LibreOffice\newline{}
          GIMP
        }
      \end{cvcolumns}

  \pagebreak[0]

  \section{Intereses}
    \cvlistitem{Programación}
    \cvlistitem{Ciencia}
    \cvlistitem{Cocina}
    \cvlistitem{Astronomía}

\end{document}
